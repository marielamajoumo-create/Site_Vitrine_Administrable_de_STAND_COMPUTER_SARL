...
async function loadFooterContact() {
  try {
    ...
    // 👇 laisse le DOM respirer après injection
    await new Promise(r => setTimeout(r, 0));

    const phoneEl = document.querySelector('.footer-contact-item.phone a');
    const emailEl = document.querySelector('.footer-contact-item.email a');
    const addressEl = document.querySelector('.footer-contact-item.address span:last-child');
        const whatsappEl = document.querySelector('.whatsapp-fab');
        const whatsappEl1 = document.querySelector('.footer-brand .social-icon[aria-label="WhatsApp"]');
    console.log(phoneEl, emailEl, addressEl, whatsappEl); // debug important

    if (!phoneEl || !emailEl || !addressEl || !whatsappEl) {
      console.error("❌ Un élément du footer est introuvable dans le DOM");
      return;
    }
    ...
    addressEl.textContent =
      `${contact.localisation}, ${contact.villeQuartier}, ${contact.pays}`;

    

  
  whatsappEl.href = "https://wa.me/" + contact.whatsapp;
  whatsappEl1.href = "https://wa.me/" + contact.whatsapp;
...
function buildFABs() {
  return `
    <a href="#" class="whatsapp-fab" target="_blank" rel="noopener" aria-label="WhatsApp">
